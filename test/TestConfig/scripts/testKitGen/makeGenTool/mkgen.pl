...
sub generateOnDir {
	...
	while ( my $entry = readdir $dir ) {
		...
		if (($javaVersion eq "SE90") || ($javaVersion eq "SE80")) {
			...
			my $latestDisabledDir = "jvmtitests proxyFieldAccess classesdbgddrext dumpromtests jep178staticLinkingTest pltest Experimental NativeTest SharedCPEntryInvokerTests gcCheck classvertest";
